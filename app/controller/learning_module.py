...
def add_user_subjects(profile_id: int, request: Request):
    ...
    try:
        ...
        return make_response(
            jsonify({
                "status": "Failed to create user subjects",
                "status_code": 401,
                "message": str(e),
                "errors": [{"field": str(e), "error": str(e)}]
            }), 
            401
        )
...
def create_user_exam(profile_id:int):
    ...
    try:
        ...
        make_response(jsonify({"error": f"An error occurred: {str(e)}"}), 401)
